...
int main()
{
	...
	while(menu != 4)
	{
		...
		printf("4 - Listar o cliente mais baixo e mais alto (cm)");
		printf("5 - Sair\n");
		scanf("%d", &menu);
		...
		else if(menu == 3)
		{
			...
			for(cliente_listar = 0 ; cliente_listar < cliente_qtd; cliente_listar++)
			{
				if(cliente_listar > 0)
				{
					if(cliente_idade[cliente_listar] > cliente_idade[cliente_idoso])
					{
						cliente_idoso = cliente_listar;
					}
					if(cliente_idade[cliente_listar] < cliente_idade[cliente_jovem])
					{
